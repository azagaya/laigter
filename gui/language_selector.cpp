	...
	: QDialog(parent), ui(new Ui::LanguageSelector)
{
	...
	if (f.open(QIODevice::ReadOnly))
	{
		...
		while (!in.atEnd())
		{
			...
			{
				icon_path = line[1];
				icon = QPixmap::fromImage(QImage(":/translations/flags/" + icon_path));
				QListWidgetItem *item = new QListWidgetItem(QIcon(icon), line[0]);
...
void LanguageSelector::on_listWidget_itemSelectionChanged()
{
	...
	QString current_language = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/lang";
...
void LanguageSelector::on_pushButton_clicked()
{
	close();
}
