...
Slider2::Slider2(QWidget *parent) : QSlider(parent)
{
...
void Slider2::setValueFromSpinBox(int v) { setValue(v * maximum() / 100.0); }
...
void Slider2::resizeEvent(QResizeEvent *ev)
{
  ...
  spin_box.resize(fm.size(Qt::TextSingleLine, num).width(), 15);
  spin_box.move(0.5 * fm.size(Qt::TextSingleLine, num).width() - 0.5 * spin_box.width(), 0);
}
