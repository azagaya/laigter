...
void OpenGlWidget::initializeGL()
{
	...
	cursorProgram.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/shaders/lvshader.glsl");
	cursorProgram.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/shaders/cursor_fragment_shader.glsl");
    ...
    /* We first check if we selected a light, and if not, we scan if we selected a texture */
		if (sample_light_list_used)
		{
			foreach (LightSource *light, *sampleLightList)
			{
				...
				if (qAbs(mouseX - lightPosition.x()) < lightWidth && qAbs(mouseY - lightPosition.y()) < lightHeight && m_light)
				{
					lightSelected = true;
					select_light(light);
					break;
		...
		if (!lightSelected)
		{
			...
			for (int i = processorList.count() - 1; i >= 0; i--)
			{
				...
				if (qAbs(mouseX - processor->get_position()->x()) < w && qAbs(mouseY - processor->get_position()->y()) < h && not selected)
				{
...
QImage OpenGlWidget::calculate_preview(bool fullPreview)
{
	...
	if (!fullPreview)
	{
		...
		foreach (ImageProcessor *processor, processorList)
		{
			...
			m_program.setUniformValue("viewPos", QVector3D(-processor->get_position()->x(), -processor->get_position()->y(), 1));
