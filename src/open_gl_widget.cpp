...
void OpenGlWidget::update_scene() {
  ...
  if (currentLightList.count() > 0) {
    foreach (LightSource *light, currentLightList) {
      ...
      transform.scale(static_cast<float>(0.3) * x, static_cast<float>(0.3) * y,
                      1);
      lightProgram.bind();
      lightVAO.bind();
...
void OpenGlWidget::mousePressEvent(QMouseEvent *event) {
      ...
      tpos.setX(
          (event->localPos().x() -
           ((processor->get_position()->x() + 1) * width() -
            processor->get_texture()->size().width() * processor->get_zoom()) *
               0.5) /
          processor->get_zoom());
    else
      ...
      tpos.setY(
          (event->localPos().y() -
           ((-processor->get_position()->y() + 1) * height() -
            processor->get_texture()->size().height() * processor->get_zoom()) *
               0.5) /
          processor->get_zoom());
    else
  ...
  if (event->buttons() & (Qt::LeftButton | Qt::MidButton)) {
    ...
    bool selected = false;
    /* We first check if we selected a light, and if not, we scan if we selected
     * a texture */
    if (sample_light_list_used) {
...
QImage OpenGlWidget::calculate_preview(bool fullPreview) {
  ...
  if (!fullPreview) {
    ...
    foreach (ImageProcessor *processor, processorList) {
      ...
      m_program.setUniformValue("viewPos",
                                QVector3D(-processor->get_position()->x(),
    ...
    foreach (ImageProcessor *processor, processorList) {
      ...
      int xi = 0.5 * (processor->get_position()->x() + 1) * width() -
               processor->get_texture()->width() / 2.0 * processor->get_zoom();
      int xf = 0.5 * (processor->get_position()->x() + 1) * width() +
               processor->get_texture()->width() / 2.0 * processor->get_zoom();
      int yi = 0.5 * (-processor->get_position()->y() + 1) * height() -
               processor->get_texture()->height() / 2.0 * processor->get_zoom();
...
void OpenGlWidget::apply_light_params() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    m_program.setUniformValue((Light + ".specColor").toUtf8().constData(),
                              color);
    m_program.setUniformValue((Light + ".diffIntensity").toUtf8().constData(),
                              light->get_diffuse_intensity());
    m_program.setUniformValue((Light + ".specIntensity").toUtf8().constData(),
