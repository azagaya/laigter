...
void ImageProcessor::generate_normal_map(bool updateEnhance, bool updateBump,
  ...
  if (rect.bottom() > texture.rect().bottom() && tileY)
    rlist.append(
        QRect(rect.left(), 0, rect.width(), rect.bottom() % texture.height())
            .intersected(texture.rect()));
  else
    diagonal = false;
  ...
  for (int i = 0; i < frames.count(); i++) {
    ...
    normals =
        (m_emboss_normal * 3 / 2.0 + m_distance_normal * 3 / 2.0 + m_height_ov);
    ...
    foreach (QRect rect, rlist) {
      ...
      for (int x = xmin; x <= xmax; ++x) {
        ...
        for (int y = ymin; y <= ymax; ++y) {
          ...
          Vec3f overlay(oColor.redF() * 2 - 1, oColor.greenF() * 2 - 1,
                        oColor.blueF() * 2 - 1);
